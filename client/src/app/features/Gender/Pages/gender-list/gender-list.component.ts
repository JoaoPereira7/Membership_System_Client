...
export class GenderListComponent {
  ...
  protected readonly data = signal<readonly GenderListItem[]>([]);
  ...
  protected readonly columns: readonly DataTableColumn<GenderListItem>[] = [
  ...
  protected readonly actions: readonly DataTableAction<GenderListItem>[] = [
    { id: 'edit', label: 'Editar', icon: 'edit', tooltip: 'Editar gênero', color: 'primary' },
  ];
            ...
            return this.service.getPaged(this.currentQuery()).pipe(
              catchError((error: unknown) => {
                ...
                this.errorMessage.set(
                  getApiErrorMessage(error, 'Não foi possível carregar os gêneros.'),
                );
                return EMPTY;
              }),
  ...
  protected onAction(event: DataTableActionEvent<GenderListItem>): void {
  ...
  private openDialog(data: GenderDialogData): void {
    this.dialog
      .open<GenderDialogComponent, GenderDialogData, GenderDialogResult>(GenderDialogComponent, {
        width: '520px',
        maxWidth: '95vw',
        autoFocus: false,
        restoreFocus: true,
