...
const normalizeLookupSearch = (value: string): string =>
  value
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
...
export class MemberDialogComponent {
  ...
  protected readonly professionSearch = signal('');
  ...
  protected readonly membershipForm = new FormGroup({
    ...
    dateJoinedChurch: new FormControl(today(), {
      nonNullable: true,
  ...
  protected get addresses(): FormArray<FormGroup> {
    return this.contactForm.controls.addresses;
  }
  protected get roles(): FormArray<FormGroup> {
    return this.rolesForm.controls.roles;
  }
  protected get departments(): FormArray<FormGroup> {
    return this.departmentsForm.controls.departments;
  }
  protected get availableChurchDepartments(): readonly ChurchDepartmentLookup[] {
    return this.churchDepartments().filter(
      (x) => x.churchId === this.membershipForm.controls.churchId.value,
    );
  }
  ...
  protected addAddress(value?: MemberAddress): void {
    this.addresses.push(
      new FormGroup({
        ...
        neighborhood: new FormControl(value?.neighborhood ?? '', {
          nonNullable: true,
          validators: requiredText,
        }),
  ...
  protected addDepartment(value?: MemberDepartment): void {
    this.departments.push(
      new FormGroup({
        id: new FormControl(value?.id ?? null),
        churchDepartmentId: new FormControl(value?.churchDepartmentId ?? '', {
          nonNullable: true,
          validators: [Validators.required],
        }),
        startDate: new FormControl(value?.startDate?.slice(0, 10) ?? today(), {
  ...
  protected remove(array: FormArray<FormGroup>, index: number): void {
  ...
  protected filteredLookup(name: string, search: string): readonly LookupItem[] {
    ...
    const items = this.lookup(name);
    return term ? items.filter((item) => normalizeLookupSearch(item.name).includes(term)) : items;
  }
  protected focusLookupSearch(input: HTMLInputElement): void {
    setTimeout(() => input.focus());
  }
  protected handleLookupSearchKeydown(event: KeyboardEvent): void {
  ...
  protected save(stepper: MatStepper): void {
    ...
    if (invalidIndex >= 0) {
      ...
      stepper.selectedIndex = invalidIndex;
      this.notification.error('Corrija os campos destacados antes de salvar.');
      return;
    }
    this.saving.set(true);
    this.dialogRef.disableClose = true;
    const request = this.buildPayload();
    const operation = this.memberId
      ? this.service.update(this.memberId, request)
      : this.service.create(request);
    operation.pipe(finalize(() => this.saving.set(false))).subscribe({
      next: (response) => {
        this.notification.success(
          this.editing ? 'Membro atualizado com sucesso.' : 'Membro cadastrado com sucesso.',
        );
        const memberId = this.memberId ?? this.extractMemberId(response);
        this.dialogRef.close({ saved: true, memberId });
      },
      error: (error) =>
        this.notification.error(getApiErrorMessage(error, 'Não foi possível salvar o membro.')),
    });
  }
      ...
      .subscribe({
        next: (data) => {
          this.generalForm.patchValue({
            ...data.member,
            birthDate: data.member.birthDate?.slice(0, 10) ?? null,
            rg: data.member.rg ?? '',
            nationality: data.member.nationality ?? 'Brasileiro',
            motherName: data.member.motherName ?? '',
            fatherName: data.member.fatherName ?? '',
            email: data.member.email ?? '',
          });
          data.phones.forEach((x) => this.addPhone(x));
          data.addresses.forEach((x) => this.addAddress(x));
          if (data.professionalInformation)
            this.professionalForm.patchValue(data.professionalInformation);
          this.membershipForm.patchValue(
            { ...data.membership, dateJoinedChurch: data.membership.dateJoinedChurch.slice(0, 10) },
            { emitEvent: false },
          );
  ...
  private loadError(error: unknown): void {
    this.loading.set(false);
    this.notification.error(
      getApiErrorMessage(error, 'Não foi possível carregar os dados da tela.'),
    );
  }
  ...
  private validateCrossFields(): void {
    ...
    future(this.generalForm.controls.birthDate);
    future(this.membershipForm.controls.dateJoinedChurch);
    const phoneValues = this.phones.controls.map((x) =>
      String(x.controls['number'].value).replace(/\D/g, ''),
    );
    this.phones.controls.forEach((x, i) => {
      if (phoneValues.indexOf(phoneValues[i]) !== i)
    ...
    validateRows(this.roles, 'churchRoleId');
    validateRows(this.departments, 'churchDepartmentId', 'activeParticipant');
    this.departments.controls.forEach((group) => {
      if (group.controls['isLeader'].value) {
        if (!group.controls['leaderTypeId'].value)
          group.controls['leaderTypeId'].setErrors({ required: true });
        if (!group.controls['leadershipStartDate'].value)
          group.controls['leadershipStartDate'].setErrors({ required: true });
      }
    });
  }
  private buildPayload(): CompleteMember {
    const g = this.generalForm.getRawValue();
    const m = this.membershipForm.getRawValue();
    ...
    return {
      ...
      member: {
        ...
        nationality: this.editing ? nullable(g.nationality) : 'Brasileiro',
        motherName: nullable(g.motherName),
        fatherName: nullable(g.fatherName),
        email: nullable(g.email),
        isActive: g.isActive,
      },
      phones: this.phones
        .getRawValue()
        .map((x) => ({ ...x, number: String(x['number']).replace(/\D/g, '') })) as MemberPhone[],
      addresses: this.addresses.getRawValue().map((x) => ({
        ...x,
        zipCode: String(x['zipCode']).replace(/\D/g, ''),
        street: String(x['street']).trim(),
        number: String(x['number']).trim(),
        complement: nullable(String(x['complement'])),
        neighborhood: String(x['neighborhood']).trim(),
      ...
      membership: {
        ...m,
        id: m.id ?? undefined,
        religiousOriginId: m.religiousOriginId,
        pastorId: m.pastorId,
      },
      membershipRoles: this.roles.getRawValue() as MembershipRole[],
      memberDepartments: this.departments.getRawValue().map((x) => ({
        id: x['id'] ?? undefined,
        churchDepartmentId: x['churchDepartmentId'],
        startDate: x['startDate'],
        endDate: x['endDate'],
        activeParticipant: !x['endDate'] && x['activeParticipant'],
        leadership: !x['isLeader']
          ? null
          : {
              id: x['leadershipId'] ?? undefined,
              leaderTypeId: x['leaderTypeId'],
              startDate: x['leadershipStartDate'],
              endDate: x['leadershipEndDate'],
              isActive: x['leadershipIsActive'],
            },
      })) as MemberDepartment[],
    };
  }
