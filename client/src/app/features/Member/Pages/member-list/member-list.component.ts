...
import {
  ...
  MemberRolesDialogResult,
} from '../../Components/member-roles-dialog/member-roles-dialog.types';
...
} from '../../Components/member-departments-dialog/member-departments-dialog.types';
...
export class MemberListComponent {
  ...
  protected readonly columns: readonly DataTableColumn<MemberListItem>[] = [
    ...
    },
    {
      ...
      width: '90px',
      align: 'center',
      formatter: (_, r) => (r.isLeader ? 'SIM' : 'NÃO'),
    },
    {
  ...
  protected readonly actions: readonly DataTableAction<MemberListItem>[] = [
    {
      id: 'membership-form',
      label: 'Ficha de Membresia',
      icon: 'description',
      tooltip: 'Visualizar e imprimir a ficha de membresia',
      color: 'primary',
    },
    { id: 'edit', label: 'Editar', icon: 'edit', tooltip: 'Editar membro', color: 'primary' },
    {
      ...
      tooltip: 'Adicionar ou remover cargos eclesiásticos',
      color: 'primary',
    },
    {
      ...
      label: 'Departamentos',
      icon: 'diversity_3',
      tooltip: 'Adicionar ou remover departamentos',
      color: 'primary',
    },
  ];
  constructor() {
  ...
  protected action(event: DataTableActionEvent<MemberListItem>): void {
    if (event.actionId === 'membership-form') {
      void this.router.navigate(['/members', event.row.id, 'membership-form']);
      return;
    }
  ...
  private openDialog(data: MemberDialogData): void {
    this.dialog
      .open<MemberDialogComponent, MemberDialogData, MemberDialogResult>(MemberDialogComponent, {
        width: '80vw',
        maxWidth: '960px',
        maxHeight: '80vh',
