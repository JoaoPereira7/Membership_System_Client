...
import { MembershipStatusDialogComponent } from '../../Components/membership-status-dialog/membership-status-dialog.component';
import {
...
@Component({
  ...
  templateUrl: './membership-status-list.component.html',
  styleUrl: './membership-status-list.component.scss',
...
export class MembershipStatusListComponent {
  ...
  protected readonly data = signal<readonly MembershipStatusListItem[]>([]);
  ...
  protected readonly columns: readonly DataTableColumn<MembershipStatusListItem>[] = [
  ...
  protected readonly actions: readonly DataTableAction<MembershipStatusListItem>[] = [
    {
      id: 'edit',
      label: 'Editar',
      icon: 'edit',
      tooltip: 'Editar situação da membresia',
      color: 'primary',
    },
  ];
            ...
            return this.service.getPaged(this.currentQuery()).pipe(
              catchError((error: unknown) => {
                ...
                this.errorMessage.set(
                  getApiErrorMessage(error, 'Não foi possível carregar as situações da membresia.'),
                );
                return EMPTY;
              }),
  ...
  protected onAction(event: DataTableActionEvent<MembershipStatusListItem>): void {
  ...
  private openDialog(data: MembershipStatusDialogData): void {
    this.dialog
      .open<
        MembershipStatusDialogComponent,
        MembershipStatusDialogData,
        MembershipStatusDialogResult
      >(MembershipStatusDialogComponent, {
        width: '520px',
