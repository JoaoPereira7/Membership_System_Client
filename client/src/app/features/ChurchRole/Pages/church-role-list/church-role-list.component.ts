...
import { ChurchRoleDialogComponent } from '../../Components/church-role-dialog/church-role-dialog.component';
import {
  ChurchRoleDialogData,
  ChurchRoleDialogResult,
...
@Component({
  ...
  templateUrl: './church-role-list.component.html',
  styleUrl: './church-role-list.component.scss',
...
export class ChurchRoleListComponent {
  ...
  protected readonly data = signal<readonly ChurchRoleListItem[]>([]);
  ...
  protected readonly columns: readonly DataTableColumn<ChurchRoleListItem>[] = [
  ...
  protected readonly actions: readonly DataTableAction<ChurchRoleListItem>[] = [
    {
      id: 'edit',
      label: 'Editar',
      icon: 'edit',
      tooltip: 'Editar cargo eclesiástico',
      color: 'primary',
    },
  ];
            ...
            return this.service.getPaged(this.currentQuery()).pipe(
              catchError((error: unknown) => {
                ...
                this.errorMessage.set(
                  getApiErrorMessage(error, 'Não foi possível carregar os cargos eclesiásticos.'),
                );
                return EMPTY;
              }),
  ...
  protected onAction(event: DataTableActionEvent<ChurchRoleListItem>): void {
  ...
  private openDialog(data: ChurchRoleDialogData): void {
    this.dialog
      .open<ChurchRoleDialogComponent, ChurchRoleDialogData, ChurchRoleDialogResult>(
        ChurchRoleDialogComponent,
        {
          width: '520px',
          maxWidth: '95vw',
          autoFocus: false,
