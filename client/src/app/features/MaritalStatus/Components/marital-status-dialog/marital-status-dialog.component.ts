...
import { MaritalStatusDialogData, MaritalStatusDialogResult } from './marital-status-dialog.types';

@Component({
  selector: 'app-marital-status-dialog',
  imports: [
    A11yModule,
    ReactiveFormsModule,
    MatButtonModule,
  ...
  styleUrl: './marital-status-dialog.component.scss',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class MaritalStatusDialogComponent {
  private readonly data = inject<MaritalStatusDialogData>(MAT_DIALOG_DATA);
  private readonly dialogRef =
    inject<MatDialogRef<MaritalStatusDialogComponent, MaritalStatusDialogResult>>(MatDialogRef);
  ...
  protected readonly title = this.isEdit ? 'Editar estado civil' : 'Novo estado civil';
  ...
  protected save(): void {
    ...
    const request: CreateMaritalStatusRequest | UpdateMaritalStatusRequest = {
      name,
    ...
    operation
      ...
      .subscribe({
        ...
        error: (error: unknown) => {
          const message = getApiErrorMessage(error, 'Não foi possível salvar o estado civil.');
  ...
  private resolveItem(): MaritalStatusListItem | null {
    if (this.data.mode === 'edit' && !this.data.item) {
      throw new Error('O estado civil é obrigatório no modo de edição.');
    }
